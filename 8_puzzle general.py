...
class Node:
    ...
    def fscore(self):
        """
        Returns the number of moves (G) and the estimated distance to the goal state, or the Manhatten distance (H)
        """
...
class Solver:
    ...
    def solve(self):
        """
        Perform heauristic search and return a path
...
class Puzzle:
    ...
    def solved(self):
        ...
        """
        goal = self.goal
        return str(self) == ''.join(map(str, goal))
    ...
    def manhattan(self):
        '''
        Determines the manhattan distance for each tile
        '''
        distance = 0
        for i in range(3):
            for j in range(3):
...
b = []
r1 = [ int(x) for x in input("Input the first row of the board. E.g. 7 2 4.\n").split()]
r2 = [ int(x) for x in input("Input the second row of the board. E.g. 5 0 6.\n").split()]
r3 = [ int(x) for x in input("Input the third row of the board. E.g. 8 3 1.\n").split()]
b.append(r1)
b.append(r2)
b.append(r3)
board = b
goal = [ int(x) for x in input("Input the order you would like the puzzle to be in. E.g. 1 2 3 4 5 6 7 8 0.\n").split()]

puzzle = Puzzle(board, goal)
